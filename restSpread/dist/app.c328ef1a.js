...
(0, _api.default)().then(function (_ref) {
  var data = _ref.data;

  var _data$results$ = data.results[0],
      email = _data$results$.email,
      cell = _data$results$.cell,
      _data$results$$name = _data$results$.name,
      title = _data$results$$name.title,
      first = _data$results$$name.first,
      otherDetails = _objectWithoutProperties(_data$results$, ["email", "cell", "name"]);
  ...
  var profile = _objectSpread({
    fullname: "".concat(title, ". ").concat(first).toUpperCase(),
    first: first,
